...
Download Android Studio from the [Official Page](http://developer.android.com/sdk/index.html). This will also install the Android SDK. Youy will need to have the Java Runtime Environment (JRE) and the Java Development Kit (JDK).

### Read this first

The following guides should be the Bible for every Android developer. Be sure to take a look at them first:

[Code Style Guidelines for Contributors](https://source.android.com/source/code-style.html)

[Best Practices](http://developer.android.com/guide/practices/index.html)
...
This structure explicitly separates 'source sets' (`main`, `androidTest`), a concept from Gradle. You could, for instance, add source sets 'paid' and 'free' into `src` which will have source code for the paid and free flavours of your app.
...
```
-keep class com.template.project.MyClass { *; }
```

To prevent ProGuard from *obfuscating* classes or class members, add a `keepnames`:
```
-keepnames class com.template.project.MyClass { *; }
```
...
This guides takes the main ideas from Futurice's android guide
